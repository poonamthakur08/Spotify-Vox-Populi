...
map_content = dbc.Card(
    ...
    ,id='map-graph', style={'background':'rgb(17, 17, 17)'}),
    className="mt-3",
)
...
stream_content = dbc.Card(
    ...
    ,id='stream-graph', style={'background':'rgb(17, 17, 17)'}),
    className="mt-3",
)

tweet_content = dbc.Card(
    html.Div(children=[
    dbc.CardBody([],id='tweet-graph', style={'background':'rgb(17, 17, 17)'} )
...
energy_bar_1 = {'width': '24%',
        'height': '32px',
        'background': '#2a9fd6',
        'border-radius': '3px 0px 0px 3px'}

energy_bar_2 = {'width': '76%',
        'height': '32px',
        'background': '#D6D6D6',
        'border-radius': '0px 3px 3px 0px'}

dance_bar_1 = {'width': '60%',
        'height': '32px',
        'background': '#2a9fd6',
        'border-radius': '3px 0px 0px 3px'}
dance_bar_2 = {'width': '40%',
        'height': '32px',
        'background': '#D6D6D6',
        'border-radius': '0px 3px 3px 0px'}

valence_bar_1 = {'width': '40%',
        'height': '32px',
        'background': '#2a9fd6',
        'border-radius': '3px 0px 0px 3px'}
valence_bar_2 = {'width': '60%',
        'height': '32px',
        'background': '#D6D6D6',
        'border-radius': '0px 3px 3px 0px'}

bar_ = {'display': 'inline-flex','margin-top': '18px','margin-bottom': '13px','width': '100%'}


visualize_song = html.Div([
        dbc.Row(
            [
                dbc.Row(html.Div(id='global-rank', children=['#25']), style=style_numbers),
                dbc.Row(html.Div('Global '), style=style_dis)
            ],
            align="center", style ={'margin-top': '-10px'}
        ),
        dbc.Row([html.Div(id='follower-count', children=['2435523'], style=style_numbers),html.Div('Number of followers', style=style_dis)]),
        
        dbc.Row([html.Div([
        html.Div(id='energy-bar-1', children='',style=energy_bar_1),
        html.Div(id='energy-bar-2', children='',style=energy_bar_2)
        ],style = bar_),html.Div('Energy', style=style_dis)]),
        
        dbc.Row([html.Div([
        html.Div(id='dance-bar-1', children='',style=dance_bar_1),
        html.Div(id='dance-bar-2', children='',style=dance_bar_2)
        ],style = bar_),html.Div('Danceability', style=style_dis)]),
    
        dbc.Row([html.Div([
        html.Div(id='valence-bar-1', children='',style=valence_bar_1),
        html.Div(id='valence-bar-2', children='',style=valence_bar_2)
        ],style = bar_),html.Div('Valence', style=style_dis)]),
...
sentiment_style = {'width': '185px', 'height': '185px','border-radius': '50%',
  'background': 'conic-gradient(red 0deg 80deg,green 80deg 190deg,#6c6c6c 190deg 360deg)','display': 'flex','align-items': 'center','justify-content': 'center'}
...
sentiment_plot = dbc.Card([
    ...
    dbc.CardBody(
        [
            html.Div([
                html.Div([],style={ 'width': '120px','height': '120px','border-radius': '50%','background':'#282828'})
            ],id='sentiment-plot',style=sentiment_style),
        ]
    ,)
])
...
tweets = dbc.Card([
    ...
    ,id='tweet-stream',style={'display':'-webkit-inline-box','overflow-x': 'auto', 'width': '100%'})
])
...
app.layout = html.Div(
    style={'backgroundColor': colors['background']}, children=[
        ...
        dbc.Row([dbc.Col(html.Div(), width=1),
                 dbc.Col(html.Div(dcc.Dropdown(artists_names,artists_names[0], id='artistdropdown', placeholder='Select Artist')),width=4),
                 dbc.Col(html.Div(dcc.Dropdown(value=track_name, id='songsdropdown', placeholder='Select Track')), width=4),
                 dbc.Col(html.Div(dbc.Button("Submit", id="submit-btn",color="primary"), className="d-grid gap-2"), width=2),dbc.Col(html.Div(), width=1)],style={'padding-top': '10px','margin-bottom': '30px','padding-bottom': '10px'}),
        
        dbc.Row([
            dbc.Col(html.Div(), width=1),
            dbc.Col(html.Div(html.Iframe(srcDoc=html_code,height='100%', width='100%'),id='spotify-player' ,style={'margin-left':'-9px','margin-right':'-9px'}),width=10),
            dbc.Col(html.Div(), width=1)
        ]),
...
@app.callback(dash.dependencies.Output('songsdropdown', 'options'),
...
@app.callback(Output('sentiment-plot', 'style'),
...
html.Img(src=img_url,style= {'margin': '10px','border-radius': '3px'}),
              html.Div(user_name,style= {'display': 'inline-block','float': 'revert','margin-left': '10px','font-weight':'1000','color':'white'}),
            html.Div('@'+user_id,style= {'display': 'inline-block','float': 'revert','margin-left': '10px'}),
              html.Div(text,style={'padding-left': '10px','padding-right': '10px','color':'#e0e0e0'}),]
             ,style= {'background': '#2F2F2F','width':'400px','height':'185px','border-radius': '3px','margin-right': '12px'}) 
    ...
    sentiment_style['background'] = 'conic-gradient(red '+str(0)+'deg '+str(neg)+'deg,green '+str(neg)+'deg '+str(pos)+'deg,#6c6c6c '+str(pos)+'deg +'+str(360)+'deg)'
...
def update_info(n_clicks, artist_name, track_name):
    ...
    for index, row in data.iterrows():
        ...
        html_code_org = '<iframe style="border-radius:3px" src="'+url+'" width="100%" height="100" margin="0px" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"></iframe>'
    ...
    fig_stream = dcc.Graph(
                figure = fig_stream
    )
    

    data_map = data[data.Country != 'Global']
    data_map['Date'] = pd.to_datetime(data_map['Date']) #data['Streams'].astype('int')#streams
    data_map = data_map.sort_values(['Date'])
    data_map['Date'] = data_map['Date'].astype('str')
    
    fig_map = px.choropleth(data_map,               
                  locations="Country",               
                  color="Streams",
    ...
    return lyrics, fig_tweet,fig_map, fig_stream, player, followers_num, rank,energy_bar_1,energy_bar_2,dance_bar_1,dance_bar_2,valence_bar_1, valence_bar_2
